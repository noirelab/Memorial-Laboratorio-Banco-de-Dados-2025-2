...
\end{itemize}
    ...
    \begin{figure}[H]
      ...
      \includegraphics[width=.9\textwidth]{MER.png}
...
\section{Agregação e Funções de Agrupamento}
	Funções de agregação como \texttt{COUNT}, \texttt{SUM}, \texttt{AVG}, \texttt{MAX} e \texttt{MIN} são usadas para resumir dados.
	O \texttt{GROUP BY} agrupa linhas para que a função de agregação seja aplicada a cada grupo, e o \texttt{HAVING} filtra os grupos.

	\begin{lstlisting}[language=SQL,caption={SELECT -- GROUP BY + COUNT: Contagem de funcionários por cargo}]
		SELECT
			   c.nome_cargo,
	...
	\begin{lstlisting}[language=SQL,caption={SELECT -- GROUP BY + SUM: Valor total em estoque por produto}]
		SELECT
			   p.nome_produto,
	...
	\begin{lstlisting}[language=SQL,caption={SELECT -- GROUP BY + HAVING: Clientes com total gasto \textgreater{} R\$ 1000}]
		SELECT
			   c.nome_cliente,
...
\section{\texttt{SELECT} nos Comandos}
	A instrução \texttt{SELECT} pode ser aninhada dentro de comandos de Data Manipulation Language (DML)
	-- \texttt{INSERT}, \texttt{UPDATE} e \texttt{DELETE} -- para fornecer valores ou critérios de forma dinâmica.

	\begin{lstlisting}[language=SQL,caption={INSERT ... SELECT -- Conceder bônus (receita extra) para todos os Vendedores}]
	...
	\begin{lstlisting}[language=SQL,caption={UPDATE -- Cancelar todas as vendas de um cliente específico}]
		UPDATE tb_venda
	...
	\begin{lstlisting}[language=SQL,caption={DELETE -- Remover todos os pagamentos associados a vendas canceladas}]
...
\section{Gatilhos}
    Os gatilhos (\textit{Triggers}) são programas armazenados, acionados automaticamente diante de eventos de modificação em uma tabela (`INSERT`, `UPDATE` ou `DELETE`).
    No sistema ``KJCF\&Cia'', um gatilho essencial foi criado para garantir a regra de negócio que determina que a quantidade de produtos deve ser alterada no estoque sempre que uma venda ocorre.

    \begin{lstlisting}[language=SQL,caption={Trigger -- Baixa automática de estoque na venda}]
        DELIMITER $$
    ...
    Também foi criado um gatilho de auditoria para impedir que o salário base de um cargo seja reduzido indevidamente, garantindo a integridade dos contratos.

    \begin{lstlisting}[language=SQL,caption={Trigger -- Impedir redução salarial}]
        DELIMITER $$
        
        CREATE TRIGGER trg_validar_alteracao_salario
        BEFORE UPDATE ON tb_cargo
        FOR EACH ROW
        BEGIN
            -- Se o novo salario for menor que o antigo, bloqueia a acao
            IF NEW.salario_base < OLD.salario_base THEN
                SIGNAL SQLSTATE '45000'
                SET MESSAGE_TEXT = 'Erro: Nao e permitido reduzir o salario base.';
            END IF;
        END $$
    ...
    \section{Eventos}
    Os eventos no MySQL funcionam como tarefas agendadas (\textit{cron jobs}) que executam comandos SQL em horários ou intervalos pré-definidos.
    Para o ``KJCF\&Cia'', implementou-se um evento diário para verificar pagamentos pendentes que ultrapassaram a data prevista, alterando seu status para 'atrasado'.

    \begin{lstlisting}[language=SQL,caption={Event -- Atualização diária de pagamentos atrasados}]
        SET GLOBAL event_scheduler = ON;

        DELIMITER $$

        CREATE EVENT ev_verificar_pagamentos_atrasados
        ON SCHEDULE EVERY 1 DAY
        STARTS '2025-08-25 00:00:00'
        DO
        BEGIN
            UPDATE tb_pagamento_cliente
...
\chapter{CONTROLE DE ACESSO}\label{chap:acesso}
    O controle de acesso em banco de dados é fundamental para garantir a segurança e a privacidade das informações. O MySQL utiliza o sistema de privilégios baseados em usuários e \textit{hosts}. As operações são geridas pelos comandos \texttt{CREATE USER}, \texttt{GRANT} e \texttt{REVOKE}.

    \section{Criar Usuários}
    Foram definidos três perfis de usuários para simular o ambiente real da empresa:
    \begin{itemize}
        \item \textbf{DBA (Admin):} Acesso total ao sistema.
        \item \textbf{Gerente:} Acesso para gerenciar funcionários, estoques e visualizar relatórios.
        \item \textbf{Vendedor:} Acesso restrito para realizar vendas e consultar produtos.
    \end{itemize}

    \begin{lstlisting}[language=SQL,caption={Criação dos Usuários}]
        -- Criacao do Administrador
        CREATE USER 'admin_db'@'localhost' IDENTIFIED BY 'senha_forte_123';

        -- Criacao do Gerente
        CREATE USER 'gerente_loja'@'%' IDENTIFIED BY 'senha_gerente';

        -- Criacao do Vendedor
        CREATE USER 'vendedor_balcao'@'%' IDENTIFIED BY 'senha_venda';
    \end{lstlisting}

    \section{Atribuir Permissões}
    Após a criação, as permissões são atribuídas de acordo com a responsabilidade de cada cargo (Princípio do Menor Privilégio).

    \begin{lstlisting}[language=SQL,caption={Atribuição de privilégios (GRANT)}]
        -- Admin: Privilegios totais com opcao de repassar permissoes
        GRANT ALL PRIVILEGES ON mydb.* TO 'admin_db'@'localhost' 
        WITH GRANT OPTION;

        -- Gerente: Pode consultar, inserir e alterar a maioria das tabelas
        -- e executar procedures
        GRANT SELECT, INSERT, UPDATE ON mydb.* TO 'gerente_loja'@'%';
        GRANT EXECUTE ON mydb.* TO 'gerente_loja'@'%';
        
        -- Vendedor: Apenas consulta produtos/clientes e insere vendas
        GRANT SELECT ON mydb.tb_produto TO 'vendedor_balcao'@'%';
        GRANT SELECT ON mydb.tb_estoque TO 'vendedor_balcao'@'%';
        GRANT SELECT, INSERT ON mydb.tb_cliente TO 'vendedor_balcao'@'%';
        GRANT INSERT ON mydb.tb_venda TO 'vendedor_balcao'@'%';
        GRANT INSERT ON mydb.tb_produto_venda TO 'vendedor_balcao'@'%';
    ...
    O comando \texttt{REVOKE} é utilizado para remover acessos indevidos ou desnecessários. Por exemplo, por questões de segurança, decidiu-se que o gerente não deve ter permissão para apagar (\texttt{DELETE}) registros, apenas consultá-los ou alterá-los. Também removemos a permissão do vendedor de alterar dados de clientes.

    \begin{lstlisting}[language=SQL,caption={Revogação de privilégios (REVOKE)}]
        -- Remove a permissao de DELETE do gerente em todas as tabelas
        REVOKE DELETE ON mydb.* FROM 'gerente_loja'@'%';

        -- Remove a permissao de UPDATE em clientes para o vendedor
        REVOKE UPDATE ON mydb.tb_cliente FROM 'vendedor_balcao'@'%';
        
        FLUSH PRIVILEGES;
    \end{lstlisting}

\chapter{TRANSAÇÕES NO BD}\label{chap:transacoes}
    As transações são sequências de operações tratadas como uma única unidade lógica de trabalho. Para garantir as propriedades ACID (Atomicidade, Consistência, Isolamento e Durabilidade), operações críticas que envolvem múltiplas tabelas devem ser encapsuladas em blocos transacionais.

    O exemplo a seguir demonstra o registro de uma venda completa. A operação envolve inserir a venda, inserir os itens vendidos e registrar o pagamento. Se qualquer etapa falhar, toda a operação é desfeita (\texttt{ROLLBACK}); caso contrário, é efetivada (\texttt{COMMIT}).

    \begin{lstlisting}[language=SQL,caption={Transação Completa de Venda (Procedure)}]
        DELIMITER $$

        CREATE PROCEDURE pr_realizar_venda_segura (
            IN p_id_cliente INT,
            IN p_id_funcionario INT,
            IN p_id_estoque INT,
            IN p_quantidade INT,
        ...
        BEGIN
            -- Declara variavel para controlar erro
            DECLARE erro_sql TINYINT DEFAULT 0;
            DECLARE v_id_venda INT;
            
            -- Handler: em caso de excecao SQL, define erro = 1
            DECLARE CONTINUE HANDLER FOR SQLEXCEPTION SET erro_sql = 1;

            START TRANSACTION;

            -- 1. Cria o registro da venda
            INSERT INTO tb_venda (id_cliente, id_funcionario, data, status)
            VALUES (p_id_cliente, p_id_funcionario, CURDATE(), 'concluida');
            
            -- Recupera o ID gerado (LAST_INSERT_ID)
            SET v_id_venda = LAST_INSERT_ID();

            -- 2. Insere o item da venda (Trigger de estoque sera acionado aqui)
            INSERT INTO tb_produto_venda (id_venda, id_estoque, quantidade, valor)
            VALUES (v_id_venda, p_id_estoque, p_quantidade, p_valor_unitario);

            -- 3. Registra o pagamento inicial
            INSERT INTO tb_pagamento_cliente 
                (id_venda, data_pagamento, forma_pagamento, status_pagamento)
            VALUES 
                (v_id_venda, CURDATE(), p_forma_pagamento, 'pago');

            -- Verifica se ocorreu erro
            IF erro_sql = 1 THEN
                ROLLBACK;
                SELECT 'Erro na transacao. Venda cancelada.' AS Mensagem;
            ELSE
                COMMIT;
    ...
    Para testar a atomicidade da transação criada acima, pode-se realizar a chamada do procedimento:
    \begin{lstlisting}[language=SQL,caption={Teste de Transação}]
        -- Tenta vender 2 itens do estoque 1 para o cliente 1
        CALL pr_realizar_venda_segura(1, 2, 1, 2, 793.78, 'pix');
    \end{lstlisting}
